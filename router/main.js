    ...
    if(!req.body.user || !req.body.pass) {
        ...
        if( req.cookies.userLogin != undefined ) {
            ...
            if(user.status == 0) {
                ...
                if(user.status == 1) {
                    res.cookie("userLogin", user.session, {maxAge: (1000*60*60*24), domain: '.srv.zenonvr.com', secure: true } );
            ...
            var user = await func.loginUser(user_name, user_pass);
            if(user.status == 1) {
                res.cookie("userLogin", user.session, {maxAge: (1000*60*60*24), domain: '.srv.zenonvr.com' } ); 
                res.json( {status: 1, data: 'Login success'} );
            }
            else {
